        ...
        <form action="{{ isset($user) ? route('admin.users.update', $user) : route('admin.users.store') }}" method="POST">
            ...
            <div class="grid grid-cols-1 gap-6">
                ...
                <div>
                    ...
                    <select id="role" name="role" {{ auth()->id() === $user->id ? 'disabled' : '' }} 
                        class="px-3 py-2 border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm w-full {{ $errors->has('role') ? 'border-red-500' : '' }} {{ auth()->id() === $user->id ? 'bg-gray-100 dark:bg-gray-800 cursor-not-allowed opacity-75' : '' }}">
                    ...
                    @if(auth()->id() === $user->id)
                        ...
                        <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">You cannot change your own role to prevent losing admin access.</p>
                    @endif
